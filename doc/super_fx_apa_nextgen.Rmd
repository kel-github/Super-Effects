...
\textcolor{blue}{Last, we sought metrics that may inform whether an experiment is likely to have yielded an imprecise effect size estimate. Effect sizes are a function of the variability of the effect across individuals, as well as within-individual variability over trials [@rouderPowerDominanceConstraint2018]. If either of these variability sources stem from a non-normal distribution, mean and standard deviation estimates -and consequently effect size computations- may be impacted by the skewness and kurtosis of these data. We thus determined whether such features of the data could predict error in effect size estimates.} 

\textcolor{blue}{Error in effect size estimates were defined for each task as the distance between the expected effect size value for $N_{313}$ and each observed effect size from the $N_{med}$ simulations. For each $N_{med}$ simulation, we also calculated the key behavioural effect for each participant and computed the Pearson's skewness and kurtosis coefficients of the resulting distribution of effects. To attain indices of the character of within participant variability, we computed the variability, skew and kurtosis from each participant's performance across trials, and then took the means of these measures across participants. The resulting variables served as predictors in a multiple regression analysis, with effect size error serving as the criterion variable. If any of these measures showed high levels of skew then a log transformation was applied. All model residuals were checked for homoscedasticity. Note also that although skewness and kurtosis share variance as they depend in part on the same parameters, performing stepwise regression yielded the same results and interpretations that we present below.} 

\textcolor{blue}{To buffer against interpreting over-fitted models, we performed k-fold cross-validation for each multiple regression model, where k=10, and we report the mean and standard deviation of the variance explained across folds. Next, we identified which regressors consistently predicted effect size error across tasks. Having identified potentially useful predictors, we then sought to identify problematic values of these predictors - i.e. what value would suggest a problematic effect size estimate. To do this, we identified the values that marked the .025 and .975 quantiles of the} N_{313} \textcolor{blue}{effect size estimates, using simple regressions, sought the predictor values for} $N_{med}$ \textcolor{blue}{that predicted effect sizes that were less than the .025 (or greater than .975) quantiles from} $N_{313}$. \textcolor{blue}{We report these values and the standard error of the prediction.}
...
get_adj_R_skew <- function(dat){
  tmp = summary(dat)
  tmp
  #tmp$adj.r.squared
}
skewR2 <- do.call(rbind, lapply(names(predictions_all_tasks),
                                function(x) get_adj_R_skew(predictions_all_tasks[[x]]$skew)))
names(skewR2) <- names(predictions_all_tasks)
```

\textcolor{blue}{Last, we determined which aspects of between and within-participant variability were predictive of imprecise effect size estimates. Multiple-regression models showed that between 10-40}% \textcolor{blue}{of the variance in effect size errors could be predicted by the skew and kurtosis of the behavioural effect across participants, as well as the mean variability, skewness and kurtosis within participants for all the behavioural effects (mean} $R^2$s (sd): AB: `r sprintf("%.2f", CV_vars["AB", "mu"])` (`r sprintf("%.2f", CV_vars["AB", "sd"])`), MT main effect: `r sprintf("%.2f", CV_vars["SD_ME", "mu"])`, (`r sprintf("%.2f", CV_vars["SD_ME", "sd"])`), MT interaction: `r sprintf("%.2f", CV_vars["SD_int", "mu"])` (`r sprintf("%.2f", CV_vars["SD_int", "sd"])`), SRT: `r sprintf("%.2f", CV_vars["SRT", "mu"])` (`r sprintf("%.2f", CV_vars["SRT", "sd"])`), CC main effect: `r sprintf("%.2f", CV_vars["CC_ME", "mu"])` (`r sprintf("%.2f", CV_vars["CC_ME", "sd"])`) \textcolor{blue}{apart from the block x condition effect from the CC task, where the model accounted for only ~1}% \textcolor{blue}{of the data. This supports the notion that non-normal sources of data may contribute to error in effect size estimates.} 

\textcolor{blue}{Across all tasks apart from the CC task, skewness and kurtosis of between-subjects effects were significant predictors of effect size error (see appendix ii). Mean within-participant skew was a significant predictor across all tasks, apart from the multitasking task x condition interaction. As the CC task data appears somewhat anomalous to the other effects under study, in both their size and distribution, we first focus on values of between-subject skew and kurtosis that commonly predicted effect size error in the remaining tasks.}

\textcolor{blue}{We next determined whether skewness or kurtosis may prove a better index of effect size imprecision, by entering each regressor separately into a simple linear regression. Apart for the multitasking condition x task interaction, between-subjects skew predicted a higher range of effect size error variance (Adjusted} $R^2$ \textcolor{blue}{s = ) than kurtosis (). This suggests that between-subjects skew may be a more informative predictor of effect size error (but see the discussion section). } 
