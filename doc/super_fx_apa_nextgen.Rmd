...
where $M_{diff}$ is the mean difference between groups, and $X_{diff}$ is one paired difference score. 
...
The AB effect was large (see Figure \@ref(fig:attEFfx)A); $N_{313}\ \epsilon_{p}^2\ M$ = `r sprintf("%.2f", AB_fx_stats[["best_mu"]])` (*SD*: `r sprintf('%.2f', AB_fx_stats[["best_sd"]])`, *LB*: `r sprintf("%.2f", AB_fx_stats[["best_LB"]])`, *UB*: `r sprintf("%.2f", AB_fx_stats[["best_UB"]])`). The simulated effect sizes for $N_{med}$ ($N_{25}$) produced the same mean effect size estimate (*M*: `r sprintf("%.2f", AB_fx_stats[["medN_mu"]])`, *SD*: `r sprintf("%.2f", AB_fx_stats[["medN_sd"]])`, *LB*: `r sprintf("%.2f", AB_fx_stats[["medN_LB"]])`, *UB*: `r sprintf("%.2f", AB_fx_stats[["medN_UB"]])`, see Figure \@ref(fig:attEFfx)B). Measures of skewness ($N_{313}$: `r sprintf("%.2f", AB_fx_stats[["best_sk"]])`, $N_{med}$: `r sprintf("%.2f", AB_fx_stats[["medN_sk"]])`) and kurtosis ($N_{313}$: `r sprintf("%.2f", AB_fx_stats[["best_ku"]])`, $N_{med}$: `r sprintf("%.2f", AB_fx_stats[["medN_ku"]])`) were close to zero and therefore indicated that the simulated distributions were comparable to a normal distribution.  With regard to extent of precision loss; the qq-ratio for $N_{med}$ was `r sprintf("%.2f", AB_fx_stats[["qq_ratio"]]["25", "2.5%"])`. The qq-ratio for small $N$ was ~3 ($N_{13}$ = `r sprintf("%.2f", AB_fx_stats[["qq_ratio"]]["13", "2.5%"])`, $N_{15}$ = `r sprintf("%.2f", AB_fx_stats[["qq_ratio"]]["15", "2.5%"])`), and reached < 2 at $N_{42}$ ($N_{36}$ = `r sprintf("%.2f", AB_fx_stats[["qq_ratio"]]["36", "2.5%"])`, $N_{42}$ = `r sprintf("%.2f", AB_fx_stats[["qq_ratio"]]["42", "2.5%"])`). The remaining qq-ratios are presented in Figure \@ref(fig:fxqq). 
...
The probability of rejecting the null hypothesis at $N_{med}$ was `r sprintf("%.2f", MT_ps_int[paste(MT_med),])`. A sample size of $N_{69}$ was required to achieve statistical power of 90 % ($N_{59}\ p$ = `r sprintf("%.2f", MT_ps_int[paste(59),])`, $N_{69}\ p$ = `r sprintf("%.2f", MT_ps_int[paste(69),])`).
...
Next we estimate the _inflation bias_ that is incurred by using a given $N$. Here we focus on the MT and CC paradigms, as they contained effects where the null was not consistently rejected at $N_{med}$. For the MT task, the task condition x modality inflation bias for $N_{med}$ was `r sprintf("%.2f", MT_tm[paste(MT_med),])`. No inflation bias was present for the main effect of task condition (all $N$ = 0). For the CC, the block x condition interaction inflation bias at $N_{med}$ was `r sprintf("%.2f", CC_bc[paste(CC_med),])`, for the main effect of condition the $N_{med}$ inflation bias was nominal (`r sprintf("%.3f", CC_c[paste(CC_med),])`). These and the remaining inflation bias estimates are presented in Figure \@ref(fig:infbias). 
...
We simulated 1000 bootstrapped experiments across 20 $N$s ranging from 13 to 313. For each paradigm and from each set of simulations, we determined the impact of $N$ on error in effect size estimates. In doing so, we were able to provide a range of effect sizes that researchers can consider when performing power analyses, particularly when using the AB, MT, SRT or CC paradigms. We also determined precision loss in effect size estimates as a function of $N$ and there were some common observations across tasks. Apart from the block x condition interaction from the CC paradigm, we found that moving from $N_{max}$ to $N_{med}$ increased the imprecision of effect size estimates by a qq-ratio of ~2; thus the range of effect sizes that may be observed across replications more or less doubles when $N_{med}$ is used (and triples when lower $N$ such as 13 or 15 are used). This observation does not depend on effect size, as this was the case for large (AB, MT tc, SRT, CC c) effects and for some small effects (MT tc x m).

This was not the case for all effects; the CC b x c interaction effect was found to be very small, and using $N_{med}$ increased the range of effect sizes that might be observed by a factor of over 4. Interestingly, for all effects, the standard deviations of the effect size distributions doubled or tripled as a function of going from $N_{max}$ to $N_{med}$, in a manner that did not disambiguate very small, small or large effects. This suggests that the extended range of effect sizes observed for CC b x c was not driven by a Gaussian process. 

The likelihood that sampling a similar study would result in (our definition) of a reasonable effect size estimate also differed for the CC paradigm, relative to the other paradigms studied here. Sampling a similar study with $N_{med}$ would produce a reasonable effect size estimate around 2/3 of the time for the AB, MT and SRT paradigms, and only 1/2 of the time for CC. This suggests that in all cases, there is the potential for misinvestment of scientific resources during study planning. If the goal of a study is to have sufficient power to reject the null hypothesis, then this only appears to be of practical issue in the study of the smaller of the effects observed here (e.g. CC b, CC b x c, MT tc x m), as the null was rejected for all remaining simulations regardless of $N$. If the goal of a study is to instead provide a reasonable effect size estimate, then a consistent observation was that higher $N$ than $N_{med}$ is required, at least for the paradigms studied here. Indeed, our analysis of inflation biases showed that, for small effects, increasing sample size results in an exponential decrease in the potential for inflated effect size estimates in the published literature. 

These findings complement previous simulation studies into the factors influencing effect size estimates. Previous simulation work has highlighted conditions that cause bias in effect size estimates [e.g. @laneEstimatingEffectSize1978; @troncososkidmoreBiasPrecisionClassical2013; @okadaOmegaSquaredLess2013] and the consequences for power calculations [@albersWhenPowerAnalyses2018; @andersonSampleSizePlanningMore2017] under simplifying conditions such as simulating between subjects experimental designs or using lower and fewer samples of $N$. By taking a step away from generality and simulating experiments based on data from specific paradigms with more complex designs, we demonstrate the utility of characterizing distributions of effect sizes. Specifically, researchers using the paradigms studied here can identify the extent to which an effect size produced from their own study may be under- or over-estimated, given we provide the means and standard deviations of the current best estimate. Furthermore, researchers can use the information presented here to evaluate findings from similar studies, by determining the likelihood that the study has produced a reasonable effect size estimate given the $N$ used by that study. These insights are unattainable from general simulation work. We hope that this study demonstrates the utility of researchers pooling data sets to improve effect size estimates for each field, consequently improving power calculations and study planning.

Compare effect sizes to literature - any meta-analyses or check the r=.36 study - also impacts of missing literature data
HERE I TALK ABOUT HOW IT'S GOOD NEWS FOR THE PARADIGMS STUDIED HERE  - largely.

Study planning - how we have helped. Lower bounds, power calculations and diminishing returns, 
Same paragraph - practicality of our findings, also, can go from effect size estimates from smaller N and divide the standrad deviation in half

How the simulation approach is one way of addressing the issue of determining relevant effect sizes for a given field

Sufficient power to produce a reasonable effect size estimates - what can we tell from what is common across the tasks in relation to this? A different way of thinking about what constitutes a well powered study. p(hit|N) - can talk about how implicit tasks are noisier here, and thus should err on larger numbers of subjects.

**on this note bit about p-values**
...
Modelling effects - how this information has helped. M

Limitations
Upper limit - a ceiling for results, also hundreds is not many
One sample - students and homogeneity
Measurement error - don't know how it will generalise
CC PARAMS 
