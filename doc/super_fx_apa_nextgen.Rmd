...
For the SRT, the best estimate of $d_z$ using $N_{313}$ was large, (Figure \@ref(fig:ILfx)A, *M*: `r sprintf("%.2f", SRT_fx_stats[["best_mu"]])`, *SD*: `r sprintf('%.2f', SRT_fx_stats[["best_sd"]])`, *LB*: `r sprintf("%.2f", SRT_fx_stats[["best_LB"]])`, *UB*: `r sprintf("%.2f", SRT_fx_stats[["best_UB"]])`). Here, there was slightly less agreement from the effect sizes observed for $N_{med}$ = 36, relative to the AB and MT (*M*, `r sprintf("%.2f", SRT_fx_stats[["medN_mu"]])`, *SD*: `r sprintf("%.2f", SRT_fx_stats[["medN_sd"]])`, *LB*: `r sprintf("%.2f", SRT_fx_stats[["medN_LB"]])`, *UB*: `r sprintf("%.2f", SRT_fx_stats[["medN_UB"]])`, see Figure \@ref(fig:ILfx)B). The qq-ratio for $N_{med}$ was `r sprintf("%.2f", SRT_fx_stats[["qq_ratio"]]["36", "2.5%"])`, suggesting that using the typical $N$ may result in greater losses of precision than was observed for the AB and MT. The remaining qq-ratios are presented in Figure [].
...
The best estimate of the block x condition interaction was on the boundary between very small and small (Figure \@ref(fig:ILfx)C, $N_{313}$ $\epsilon_{p}^2$ *M*: `r sprintf("%.2f", CC_fx_stats_int[["best_mu"]])`, *SD*: `r sprintf("%.2f", CC_fx_stats_int[["best_sd"]])`, *LB*: `r sprintf("%.2f", CC_fx_stats_int[["best_LB"]])`, *UB*: `r sprintf("%.2f", CC_fx_stats_int[["best_UB"]])`). There was a minor discrepancy between the $N_{313}$ and $N_{med}$ (25) *M*s, but not the *Mdn*s (*M*: `r sprintf("%.2f", CC_fx_stats_int[["medN_mu"]])`, *Mdn*: `r sprintf("%.2f", CC_fx_stats_int[["medN_med"]])`, *SD*: `r sprintf("%.2f", CC_fx_stats_int[["medN_sd"]])`). Similar to the SRT task, interpreting effect size distributions for $N_{med}$ would result in different conclusions to $N_{313}$ as the latter crossed zero ($N_{med}$ *LB*: `r sprintf("%.2f", CC_fx_stats_int[["medN_LB"]])`, *UB*: `r sprintf("%.2f", CC_fx_stats_int[["medN_UB"]])`). There was also a greater extent of precision loss at $N_{med}$ than was observed for other tasks (qq-ratio: `r sprintf("%.2f", CC_fx_stats_int[["qq_ratio"]]["25", "2.5%"])`)). The remaining qq-ratios are presented in Figure [].

##### Main Effect of Condition

The main effect of condition was large ($N_{313}$ $\epsilon_{p}^2$ *M*: `r sprintf("%.2f", CC_fx_stats_ME[["best_mu"]])`, *SD*: `r sprintf("%.2f", CC_fx_stats_ME[["best_sd"]])`, *LB*: `r sprintf("%.2f", CC_fx_stats_ME[["best_LB"]])`, *UB*: `r sprintf("%.2f", CC_fx_stats_ME[["best_UB"]])`, see Figure \@ref(fig:ILfx)E). There was a minor discrepancy between the $N_{313}$ and $N_{med}$ (25) *M*s, and *Mdn*s (*M*: `r sprintf("%.2f", CC_fx_stats_ME[["medN_mu"]])`, *Mdn*: `r sprintf("%.2f", CC_fx_stats_ME[["medN_med"]])`, *SD*: `r sprintf("%.2f", CC_fx_stats_ME[["medN_sd"]])`, *LB*: `r sprintf("%.2f", CC_fx_stats_ME[["medN_LB"]])`, *UB*: `r sprintf("%.2f", CC_fx_stats_ME[["medN_UB"]])`). There was comparable precision loss as was observed for the SRT (qq-ratio: `r sprintf("%.2f", CC_fx_stats_int[["qq_ratio"]]["25", "2.5%"])`). The remaining qq-ratios are presented in Figure [].
