...
\textcolor{blue}{Last, we determined which aspects of the data were predictive of erroneous effect size estimates. Multiple-regression analysis showed that between ~10-40}% \textcolor{blue}{of the variance in effect size errors were predicted by effect skewness, effect kurtosis, mean intra-individual variance, mean intra-individual skewness, and mean intra-individual kurtosis (mean cross-validated} $R^2$s (sd): AB: `r sprintf("%.2f", CV_vars["AB", "mu"])` (`r sprintf("%.2f", CV_vars["AB", "sd"])`), MT main effect of task: `r sprintf("%.2f", CV_vars["SD_ME", "mu"])`, (`r sprintf("%.2f", CV_vars["SD_ME", "sd"])`), MT task x modality interaction: `r sprintf("%.2f", CV_vars["SD_int", "mu"])` (`r sprintf("%.2f", CV_vars["SD_int", "sd"])`), SRT: `r sprintf("%.2f", CV_vars["SRT", "mu"])` (`r sprintf("%.2f", CV_vars["SRT", "sd"])`), CC main effect of condition: `r sprintf("%.2f", CV_vars["CC_ME", "mu"])` (`r sprintf("%.2f", CV_vars["CC_ME", "sd"])`), all ps < .001), \textcolor{blue}{apart from for the block x condition effect from the CC task, where the model accounted for a negligible proportion (~1}% \textcolor{blue}{) of effect size error (F(} `r sprintf("%d", ccintstats$fstatistic["numdf"])`,`r sprintf("%d", ccintstats$fstatistic["dendf"])` \textcolor{blue}{) =} `r sprintf("%.2f", ccintstats$fstatistic["value"])` \textcolor{blue}{p = .04). This suggests that skewness and kurtosis of effects and within-participant variability predict reasonable variability in effect size error, apart from when effect sizes are very small, such as is the case the CC block x condition interaction (} $\epsilon_{p}^2$ ~ `r sprintf("%.2f", CC_fx_stats_int[["best_LB"]])` - `r sprintf("%.2f", CC_fx_stats_int[["best_UB"]])` \textcolor{blue}{). We therefore no longer consider this latter effect in the subsequent analysis}. 

\textcolor{blue}{The resulting regression equations (see appendix ii) are useful for researchers using the tasks studied here, who wish to predict the extent to which their own experiment may have yielded error in the effect size estimate. However, what is more widely useful is understanding which regressors significantly predict effect sizes across tasks, and which are therefore likely to predict effect size error across variants of the current tasks or other tasks assessing cognition. We therefore determined which regressors showed significant predictive power across tasks, applying Bonferonni correction for multiple comparisons. For the AB, MT, and SRT tasks, effect skewness and kurtosis were significant predictors of effect size error (all ps <= .005, see appendix ii). Mean intra-individual skew was a significant predictor across all four tasks (all ps < .008), apart from for the MT task x condition interaction (p=.08). This suggests that examining the effect skewness and kurtosis and mean intra-individual skew of a single dataset may provide clues as to the extent to which there may be error in the resulting effect size estimate.

Having identified the regressors that may suggest error in effect size estimates across tasks, we next sought to determine whether each regressor could be used as a marker of effect size error in the absence of holding other predictors constant. Such a finding would suggest that use of a single piece of information (e.g. effect skewness) could be sufficient to indicate the extent to which a single experiment has yielded an imprecise effect size estimate. Simple regressions between each predictor and effect size errors showed that effect skewness tended to predict a higher proportion of variance in effect size error (Adjusted} $R^2$ \textcolor{blue}{s:} `r sprintf("%.2f", skewR2["SRT", 1])` - `r sprintf("%.2f", skewR2["AB", 1])`, all ps < .001) \textcolor{blue}{than kurtosis (Adjusted} $R^2$: `r sprintf("%.2f", kurR2["SD_ME", 1])` - `r sprintf("%.2f", kurR2["SRT", 1])`, all ps < .7), \textcolor{blue}{apart from for the multitasking condition x task interaction (skewness: Adjusted} $R^2$ \textcolor{blue}{:} `r sprintf("%.2f", skewR2["SD_int", 1])` \textcolor{blue}{p<.01) kurtosis: Adjusted} $R^2$ \textcolor{blue}{:} `r sprintf("%.2f", kurR2["SD_int", 1])` \textcolor{blue}{p<.001). Although mean intra-individual skewness predicted higher amounts of error variance for the AB (Adj} $R^2$: `r sprintf("%.2f", muskewR2["AB", 1])` \textcolor{blue}{) and CC main effect of condition (Adj} $R^2$: `r sprintf("%.2f", muskewR2["CC_ME", 1])` \textcolor{blue}{), its predictive power was poor for the remaining tasks (Adj} $R^2$ \textcolor{blue}{s: <= .02, all ps < .17. This suggests that effect skewness is a better predictor of effect size error, when not controlling for other influences.}

\textcolor{blue}{U2H SOME INTRO skewness predicts 5-10}% \textcolor{blue} of variance in effect size error, we next determined which values of effect skewness serve as a marker for problematic effect size error. Where skewness was a better predictor of effect size error (AB, SRT, and MT main effect of condition), moderate to large negative effect skewness (VALS) was suggestive of erroneous over-estimates of effect size, whereas large positive skew was predictive of erroneous under-estimates (VALS). Thus, WRAP THIS UP OF WHY ITS IMPORTANT.}
