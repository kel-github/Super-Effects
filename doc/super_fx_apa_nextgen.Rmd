...
We simulated 1000 bootstrapped experiments across 20 $N$s ranging from 13 to 313. For each paradigm and from each set of simulations, we determined the impact of $N$ on error in effect size estimates. In doing so, we were able to provide a range of effect sizes that researchers can consider when performing power analyses for studies, particularly when using the AB, MT, SRT or CC paradigms. There were some common observations across tasks; apart from the block x condition interaction from the CC paradigm, we found that for the remaining effects that moving from $N_{max}$ to $N_{med}$ increased the imprecision of effect size estimates by a factor of ~2; thus the range of effect sizes that may be observed across replications more or less doubles when $N_{med}$ is used (and triples when lower $N$ such as 13 or 15 are used). This is the case for large (AB, MT tc, SRT, CC c) effects and for some small effects (MT tc x m). 

This was not the case for all effects; the CC b x c interaction effect was found to be very small, and using $N_{med}$ increased the range of effect sizes that might be observed by a factor of over 4. Interestingly, for all effects, the standard deviations of the effect size distributions doubled or tripled as a function of going from $N_{max}$ to $N_{med}$, in a manner that did not disambiguate very small, small or large effects. This suggests that the expanded qq-ratio for CC b x c was not driven by a Gaussian process. NEXT: Then p(hit|N), and the inflated estimates. 

Relate findings to other simulation work. MEASURMENT ERROR HELD CONSTANT HERE
