...
The AB effect was large (see Figure \@ref(fig:attEFfx)A); the mean (*M*) estimate of $\epsilon_{p}^2$ using $N_{313}$ was `r sprintf("%.2f", AB_fx_stats[["best_mu"]])` (*SD*: `r sprintf('%.2f', AB_fx_stats[["best_sd"]])`, *LB*: `r sprintf("%.2f", AB_fx_stats[["best_LB"]])`, *UB*: `r sprintf("%.2f", AB_fx_stats[["best_UB"]])`). There was agreement from the simulated effect sizes for $N_{med}$ (*M*: `r sprintf("%.2f", AB_fx_stats[["medN_mu"]])`), with the expected decrease in precision (*SD*: `r sprintf("%.2f", AB_fx_stats[["medN_sd"]])`, *LB*: `r sprintf("%.2f", AB_fx_stats[["medN_LB"]])`, *UB*: `r sprintf("%.2f", AB_fx_stats[["medN_UB"]])`, see Figure \@ref(fig:attEFfx)B). The qq-ratio for $N_{25}$ was `r sprintf("%.2f", AB_fx_stats[["qq_ratio"]]["25", "2.5%"])`; [WHAT HERE] $N$ may result only in modest losses of precision. The remaining qq-ratios are presented in Figure \@ref(fig:fxqq). The qq-ratio for small $N$ was ~3 ($N_{13}$ = `r sprintf("%.2f", AB_fx_stats[["qq_ratio"]]["13", "2.5%"])`, $N_{15}$ = `r sprintf("%.2f", AB_fx_stats[["qq_ratio"]]["15", "2.5%"])`), and reached under 2 at $N_{42}$ ($N_{36}$ = `r sprintf("%.2f", AB_fx_stats[["qq_ratio"]]["36", "2.5%"])`, $N_{42}$ = `r sprintf("%.2f", AB_fx_stats[["qq_ratio"]]["42", "2.5%"])`).
...
Next we computed p(hit|$N$); an estimate of the impact of imprecision in effect size estimates when basing power calculations on a similar study with $N_{med}$. For the AB, MT and SRT paradigms, the p(hit|$N_{med}$) was ~0.67 (AB: `r sprintf("%.2f", phit_dat$p_upper[phit_dat$mod == "AB" & phit_dat$n == AB_med])`, MT tc: `r sprintf("%.2f", phit_dat$p_upper[phit_dat$mod == "tt" & phit_dat$n == MT_med])`, MT tc x m: `r sprintf("%.2f", phit_dat$p_upper[phit_dat$mod == "ta*tt" & phit_dat$n == MT_med])`, SRT: `r sprintf("%.2f", phit_dat$p_upper[phit_dat$mod == "SRT" & phit_dat$n == SRT_med])`. This suggests that sampling a similar study will produce a reasonable _a prior_ effect size estimate 2/3 of the time (Note: it is interesting that the AB, MT and SRT fields appear to have converged on an $N_{med}$ that puts them on a comparable footing for hitting the best effect size. Indeed, if the MT and SRT fields used the same sample size as the AB field, the p(hit|$N_{25}$) ratios for the three effects would be < .6 (tc: `r sprintf("%.2f", phit_dat$p_upper[phit_dat$mod == "tt" & phit_dat$n == AB_med])`, tc x m: `r sprintf("%.2f", phit_dat$p_upper[phit_dat$mod == "ta*tt" & phit_dat$n == AB_med])`, SRT: `r sprintf("%.2f", phit_dat$p_upper[phit_dat$mod == "SRT" & phit_dat$n == AB_med])`. We see below that this is unlikely to be related to statistical power to reject the null hypothesis). For the CC paradigm, the p(hit|$N_{med}$ = ~.5 (b x c: `r sprintf("%.2f", phit_dat$p_upper[phit_dat$mod == "b*c" & phit_dat$n == CC_med])`, c: `r sprintf("%.2f", phit_dat$p_upper[phit_dat$mod == "c" & phit_dat$n == CC_med])`). This suggests that applying this approach to plan a CC study will result in an adequately powered study 50% of the time. The remaining p(hit|$N_x$) are presented in Figure \@ref(fig:fxphit).
...
Next we estimate the _inflation bias_ that is incurred by using a given $N$ . Here we focus on the MT and CC paradigms, as they were the two paradigms with effects where the null was not consistently rejected at $N_{med}$. The inflation bias estimates are presented in Figure \@ref(fig:infbias). For the MT task, the inflation bias for $N_{med}$ was `r sprintf("%.2f", MT_tm[paste(MT_med),])` ($\epsilon_{p}^2$) for the task condition x modality interaction at $N_{med}$, which is equivalent to a small effect size. The inflation bias became equivalent to being very small ($\epsilon_{p}^2$ = .02, @cohenPowerPrimer1992), at $N_{59}$. No inflation bias was present for the main effect of task condition (all $N$ = 0). For CC, the inflation bias at $N_{med}$ for the block x condition interaction was `r sprintf("%.2f", CC_bc[paste(CC_med),])`, which again is comparable a small effect size. The inflation bias became comparable to a very small effect size at $N_{30}$ (inflation bias: `r sprintf("%.2f", CC_bc["30",])`). For the main effect of condition the $N_{med}$ inflation bias was nominal (`r sprintf("%.3f", CC_c[paste(CC_med),])`).
