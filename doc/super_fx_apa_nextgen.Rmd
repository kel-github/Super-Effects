...
AIMS:
1. Provide a good estimate/quantification of the key effect sizes (and range) of effect sizes that can be expected to be observed. Thereby providing an anchor to motivate power calculations for future work.
2. To identify the consequence of sample size on effect size estimates; i.e. how much error there may be in effect size estimates  (fx size figure and probability curves)
3. To use the simulation approach to identify circumstances where there may be missing information about effect sizes, which could potentially hamper meta-analytic approaches.
4. To determine across tasks, if there is a way to identify an optimal N for common EF and IL tasks.
