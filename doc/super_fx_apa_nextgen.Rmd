...
\textcolor{blue}{Last, we determined which aspects of the data were predictive of erroneous effect size estimates. Multiple-regression models showed that between ~10-40}% \textcolor{blue}{of the variance in effect size errors could be predicted by effect skewness, effect kurtosis, mean intra-individual variance, skewness, and kurtosis (mean} $R^2$s (sd): AB: `r sprintf("%.2f", CV_vars["AB", "mu"])` (`r sprintf("%.2f", CV_vars["AB", "sd"])`), MT main effect: `r sprintf("%.2f", CV_vars["SD_ME", "mu"])`, (`r sprintf("%.2f", CV_vars["SD_ME", "sd"])`), MT interaction: `r sprintf("%.2f", CV_vars["SD_int", "mu"])` (`r sprintf("%.2f", CV_vars["SD_int", "sd"])`), SRT: `r sprintf("%.2f", CV_vars["SRT", "mu"])` (`r sprintf("%.2f", CV_vars["SRT", "sd"])`), CC main effect: `r sprintf("%.2f", CV_vars["CC_ME", "mu"])` (`r sprintf("%.2f", CV_vars["CC_ME", "sd"])`), all ps < .001), \textcolor{blue}{apart from the block x condition effect from the CC task, where the model accounted for a negligible proportion (~1}% \textcolor{blue}{) of effect size error (F(U2H ADD STAT DETAILS} . This suggests that non-normal sources of data predict error in effect size estimates, apart from when XXXXX.} 

\textcolor{blue}{For the AB, MT, and SRT tasks, effect skewness and kurtosis were significant predictors of effect size error (all ps < see appendix ii). Mean intra-individual skew was a significant predictor across all four tasks, apart from for the MT task x condition interaction. We next determined which of these regressors may serve as a predictor of error when used as a marker in isolation. Effect skewness tended to predict a higher range of effect size error variance (Adjusted} $R^2$ \textcolor{blue}{s:} `r sprintf("%.2f", skewR2["SRT", 1])` - `r sprintf("%.2f", skewR2["AB", 1])`) \textcolor{blue}{than kurtosis (Adjusted} $R^2$: `r sprintf("%.2f", kurR2["SD_ME", 1])` - `r sprintf("%.2f", kurR2["SRT", 1])`), \textcolor{blue}{apart from for the multitasking condition x task interaction (skewness: Adjusted} $R^2$ \textcolor{blue}{:} `r sprintf("%.2f", skewR2["SD_int", 1])` \textcolor{blue}{kurtosis: Adjusted} $R^2$ \textcolor{blue}{:} `r sprintf("%.2f", kurR2["SD_int", 1])` \textcolor{blue}{). Although mean intra-individual skewness predicted higher amounts of error variance for the AB (Adj} $R^2$: `r sprintf("%.2f", muskewR2["AB", 1])` \textcolor{blue}{) and CC main effect of condition (Adj} $R^2$: `r sprintf("%.2f", muskewR2["CC_ME", 1])` \textcolor{blue}{), its predictive power was poor for the remaining tasks (Adj} $R^2$ \textcolor{blue}{s: <= .02. This suggests that effect skewness is a more informative predictor of effect size error, when not controlling for other influences.}
