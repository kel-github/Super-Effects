...
We next determined how effect size observations differ when experiments use an $N$ that is typical for the field. We report the same summary statistics as above, for the $N$ that is closest to the median sample size used in the field for that task ($N_{med}$). To identify the median $N$, we conducted a survey of the recent literature (see below). We computed _precision loss_ from using $N_{med}$ by taking the ratio between the UB ($\frac{LB_{N_{med}} }{LB_{N_{313}}}$) or LB quantiles ($\frac{LB_{N_{313}} }{LB_{N_{med}}}$). We refer to these from now as the UB and LB ratios. Note that the numerator and denominator are switched between the LB and the UB ratios, so that a value larger than 1 indicates how much higher or lower the $N_{med}$ bound is, relative to the bound for $N_{313}$. We also present LB and UB ratios across all sampled $N$s, to provide an idea of how much may be gained in precision if the field were to increase the typical $N$. Note that although we expect these ratios to be some function of $\frac{1}{N}$ (given that variance is a function of this term), the exact relationship between $N$ and precision will be dependent on the population variance and measurement error. 
...
To determine the $N$ required to achieve 90% power to reject the null hypothesis, we report the $N$ for which over 90% of p-values pass the threshold for significance for the effects of interest ($\alpha$=.05). To assess how confidence in p-values, or certainty of the test outcome, changes with $N$, we present the same statistics for each $N$ (mean, standard deviation, UB, LB and qq-ratios). As p-values clustered close to 0 in many instances, this measure will be subject to some floor effects but should also determine where clear information gains are available by increasing $N$. Additionally, because p-values cluster close to 0, we applied the probit transform to rescale the values on the range [-$\infty$, $\infty$] for visualisation purposes only. 
...
get_stats_4_fx <- function(dat, task, mod, meas, medN, ns){
  ...
  LB_ratio <- do.call(rbind, lapply(ns, function(x) 
    sqrt(dat$LB[dat$task == task & dat$mod == mod & dat$meas == meas & dat$n == 313]^2/
         dat$LB[dat$task == task & dat$mod == mod & dat$meas == meas & dat$n == x]^2)))
  
  UB_ratio <- do.call(rbind, lapply(ns, function(x) 
    sqrt(dat$UB[dat$task == task & dat$mod == mod & dat$meas == meas & dat$n == x]^2/
         dat$UB[dat$task == task & dat$mod == mod & dat$meas == meas & dat$n == 313]^2)))
  
  out <- list(best_med, best_sd, best_LB, best_UB, medN_med, medN_sd, medN_LB, 
              medN_UB, LB_ratio, UB_ratio)
  names(out) <- c("best_med", "best_sd", "best_LB", "best_UB", "medN_med", "medN_sd",
                  "medN_LB", "medN_UB", "LB_ratio", "UB_ratio")
  out
...
Overall, the AB effect was large and reducing $N$ to the $N_{med}$ had negligible impacts on effect size estimation; the median estimate of $\epsilon_{p}^2$ using $N_{313}$ was `r sprintf("%.2f", AB_fx_stats[["best_med"]])` (sd: `r sprintf('%.2f', AB_fx_stats[["best_sd"]] )`). For $N_{med}=25$ the median $\epsilon_{p}^2$ was identical (`r sprintf("%.2f", AB_fx_stats[["medN_med"]])`, sd: `r sprintf("%.2f", AB_fx_stats[["medN_sd"]])`). The LB and UB quantiles were broadly comparable between $N_{313}$ [LB: `r sprintf("%.2f", AB_fx_stats[["best_LB"]])`, UB: `r sprintf("%.2f", AB_fx_stats[["best_UB"]])`] and $N_{med}$ [LB: `r sprintf("%.2f", AB_fx_stats[["medN_LB"]])`, UB: `r sprintf("%.2f", AB_fx_stats[["medN_UB"]])`], with ratios of `r sprintf("%.2f", AB_fx_stats[["best_LB"]]/AB_fx_stats[["medN_LB"]])` and `r sprintf("%.2f", AB_fx_stats[["medN_UB"]]/AB_fx_stats[["best_UB"]])` respectively. The worst case LB and UB ratios were also fairly low; if the field used $N$=13, then the lowest effect size estimates would be up to `r sprintf("%.2f", AB_fx_stats[["LB_ratio"]][[1]])` times the lower bound, whereas the largest would be `r sprintf("%.2f", AB_fx_stats[["UB_ratio"]][[1]])` times the upper bound. The remaining LB and UB ratios are presented in Figure [].
