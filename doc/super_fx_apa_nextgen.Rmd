...
_Effect Sizes_
For each task, we report the following information from the effect size densities that were produced by simulating 1000 experiments each at each level of $N$; first we determined the most precise estimate of the range of effect sizes for each task that we could attain given our data. We therefore report the median (M), standard deviation (SD) and the .025 (lower bound, LB) and .975 (upper bound, UB) quantiles from the effect size distribution from our highest $N$ ($N_{313}$). These values can be used to define, _a priori_, the range of anticipated effect sizes for future experiments, and consequently, can motivate power calculations for future studies. 

We next determined how the range of observed effect sizes differs from the most precise estimate when experiments use an $N$ that is typical for the field. We report the same summary statistics as above, for the $N$ that is representative of the median sample size actually used in the field ($N_{rep}$), which we calculated from a survey we conducted of the recent literature (see below). We compute how much precision is lost when using $N_{rep}$ by taking the ratio between the UB or LB quantiles (e.g. $\frac{LB_{N_{rep}} }{LB_{N_{313}}}$). We refer to these from now as the LB and UB ratios. 

Next we determined the point of diminishing returns; i.e. where increasing sample size offered little gains in the precision of effect size estimates over experiments. To achieve this, we fit piecewise linear functions 

we fit the following piecewise linear function to the UB and LB estimates:
