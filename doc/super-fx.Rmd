...
_Effect sizes_ The observed $\eta_{p}^2$ densities are presented in Figure \@ref(fig:CCFX) for the block x condition interaction (A) and for the main effect of condition (B). There was disparity between the median effect sizes observed for the block x condition interaction for $N_{25}$ (median $\eta_{p}^2$ = `r sprintf("%.2f", res['25', 'stats_fx'][[1]][,"RM-AN"]$med)`, sd: `r sprintf("%.2f", res['25', 'stats_fx'][[1]][,"RM-AN"]$sd)`) and $N_{313}$ (median $\eta_{p}^2$ = `r sprintf("%.2f", res['313', 'stats_fx'][[1]][,"RM-AN"]$med)`, sd: `r sprintf("%.2f", res['313', 'stats_fx'][[1]][,"RM-AN"]$sd)`). In contrast for the main effect of condition, the median effect size was consistent between $N_{25}$ and $N_{313}$  ($N_{25}$ median $\eta_{p}^2$ = `r sprintf("%.2f", res['25', 'stats_fx'][[1]][,"LME"]$med)`, sd: `r sprintf("%.2f", res['25', 'stats_fx'][[1]][,"LME"]$sd)`, $N_{313}$ median $\eta_{p}^2$ = `r sprintf("%.2f", res['313', 'stats_fx'][[1]][,"LME"]$med)`, sd: `r sprintf("%.2f", res['313', 'stats_fx'][[1]][,"LME"]$sd)`). However, there was some discrepancy when the mean of the $\eta_{p}^2$ was taken for the main effect of condition ($N_{25}$ mean = `r sprintf("%.2f", res['25', 'stats_fx'][[1]][,"RM-AN"]$mu)` vs $N_{313}$ = `r sprintf("%.2f", res['313', 'stats_fx'][[1]][,"RM-AN"]$mu)`. The Q-Q plots further revealed the disparity between the densities for both the block x condition interaction and the main effect of condition (Figure \@ref(fig:CCFX), panel C). Specifically, the relationship between both sets of densities looks non-linear, suggesting that the distribution of effect sizes yielded from experiments with $N_{25}$ carries different information to those yielded from experiments of $N_{313}$. Furthermore, examination of the Q-Q plot shows that there is little overlap between the densities for the block x condition interaction. Taken together, these findings suggest that aggregating across typical experiments on the field is unlikely to motivate an accurate estimate of the true effect size. Rather, this can only be gained if the field uses experiments with larger $N$, at least when modelled using repeated measures ANOVA. In support of this interpretation, examination of the $D_{KL}$ between the density of each $N$ and $N_{313}$ shows larger losses of information for lower $N$, with information gain saturating at around $N_{160}$ for both the main effect of condition and the block x condition interaction (Figure \@ref(fig:CCFX), panel D). This suggests significant information loss when using single experiments of lower N to produce effect size estimates for CC. Moreover, meta-analytic (p<.05) effect sizes were larger than observed effect sizes up until $N_{115}$, suggesting that a current meta-analysis of the field would result in an inflated effect size estimate.
