...
The brain is the most complex organism known to humans, yet decision making regarding theory about its function is often made on binary (i.e. pass or fail) terms, across the psychological, neuroscientific and biomedical sciences [@szucsWhenNullHypothesis2017]. Theory is often specified in terms of the the presence or absence of a given effect, and a yes/no decision is made about whether the null hypothesis (absence of an effect) can be rejected. An alternate approach is to develop theory and models that attempt to quantify the extent to which an effect should be observed, i.e. the anticipated effect size. The latter prediction is more risky, and therefore constitutes a more desirable prediction for theory testing [@popperLogicScientificDiscovery2005]. Considering the complexity of the human brain, it indeed seems unlikely that such pass/fail decision-making will be sufficient to disentangle the myriad functional systems that the brain has developed over millions of years of evolution. However, to move towards theories that can be quantified to predict anticipated changes in the size of an experimental effect, it is essential that we gain an understanding of exactly how much insight is gained from our current effect size estimates; i.e. how well are we currently quantifying effect sizes? Here we address this question focusing on established paradigms from experimental psychology that tap executive function and implicit learning. 

Quantifying the range of the effect sizes that is likely to observed across multiple experiments from a given field (e.g. a set of experiments using a given paradigm) can motivate theory development beyond knowing the most likely size of the experimental effect. In Bayesian approaches, effect sizes are often assumed to drawn from a Cauchy distribution [@gelmanWeaklyInformativeDefault2008, @liangMixturesPriorsBayesian2008, @moreyBayesFactorApproaches2011, @rouderBayesianTestsAccepting2009, @rouderDefaultBayesFactors2012a, @rouderDefaultBayesFactors2012b]. This is with good reason as the prior x.    

Theoretical insight: often assumed to be cauchy. This tells us 1) the shape - knowing the probability of attaining a range of effect sizes helps to understand the nature of effect under study - is it dispersed - i.e. sensitive to fluctuating variability, unimodal, multimodal? Sampling multiple subtypes for example, executive function subtypes, or implicit learning questions. [theoretical assumption is that effect sizes often follow a cauchy and good reason, however do we know that this is borne out in our experimental data from potentially heterogenous populations] would be good here to add some refs about EF and IL being touted as cognitive markers.

Theory significance: 2) the most probable effect size (or the best estimate) of the effect size at each $N$ - we can know a) how big is the effect - how pertinent is it in the population, but also b) with lower $N$ can we estimate the effect we care about in due course. e.g. some effects are clearly robust (EF).  

Computation: 3) convergence between effect size densities for higher vs lower $N$, and how much information is lost when using lower $N$ effect sizes as a proximation (e.g. meta-analysis) - this tells us how much taking a handful of studies might lead us to a poorly motivated power calculation.

Bias and computation: 4) meta-analyses are biased by the published results. We don't know how much such attempts to quantify effect size is distorted by this issue. Time potentially wasted by researchers if inflated effect sizes are causing them to run underpowered studies.

Certainty in decision-making: Effect and $N$ related to $p$; given the above, the certainty we have over binary decision making is low. What is the actual power given various $N$ and how sure can we be about that?

Aims and method Things to add
- table
